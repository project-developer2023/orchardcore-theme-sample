...
function createAssetGroupTask(assetGroup, doRebuild) {
    ...
    if (assetGroup.copy === true) {
        ...
        switch (outputExt) {
            ...
            case ".html":
                return buildHtmlPipeline(assetGroup, doConcat, doRebuild);
        }
    }
}
...
function buildHtmlPipeline(assetGroup, doConcat, doRebuild) {
    ...
    return gulp.src(assetGroup.inputPaths) // Non-minified output, with source mapping
        .pipe(fileinclude({
              prefix: '@@',
              basepath: '@file'
            }))
